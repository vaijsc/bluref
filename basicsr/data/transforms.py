...
def paired_random_crop_conf(inp, gt_patch_size, scale, gt_path):
    """Paired random crop.
    ...
    images_lqs, images_gts, images_conf = [], [], []
    for _ in range(4):
        ...
        images_lqs += img_lqs_temp
        images_gts += img_gts_temp
        images_conf += img_conf_temp

    if len(images_gts) == 1:
        images_gts = images_gts[0]
    if len(images_lqs) == 1:
        images_lqs = images_lqs[0]
    # breakpoint()
    return images_gts, images_lqs, images_conf
...
def augment(imgs, hflip=True, rotation=True, flows=None, return_status=False, vflip=False):
    ...
    aug = iaa.MotionBlur(k=15, angle=(0, 360))

    def _augment(img, idx):
        # if idx == 2:
            # breakpoint()
        if hflip:  # horizontal
            # if idx == 2: cv2.flip(img, 1, img)
            img = cv2.flip(img, 1)
            if len(img.shape) == 2:
                img = img[:, :, np.newaxis]
            # cv2.flip(img, 1, img)
            if img.shape[2] == 6:
                img = img[:,:,[3,4,5,0,1,2]].copy() # swap left/rightr 
        if vflip:  # vertical
            # cv2.flip(img, 0, img)
            img = cv2.flip(img, 0)
            if len(img.shape) == 2:
    ...
    if not isinstance(imgs, list):
        imgs = [imgs]
    # breakpoint()
    imgs = [[_augment(each, idx) for each in img] for idx, img in enumerate(imgs)]
    if len(imgs) == 1:
        imgs = imgs[0]

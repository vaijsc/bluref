...
from basicsr.utils import (MessageLogger, check_resume, get_env_info,
                           ...
                           set_random_seed, tensor2img, img2tensor)
from basicsr.utils.dist_util import get_dist_info, init_dist
from basicsr.utils.options import dict2str, parse
from basicsr.models.densematching.model_selection import select_model
...
def main():
    ...
    # DenseMatching Initialization
    dense = opt['densematching']
    network, estimate_uncertainty = select_model(
            dense['model'], dense['pre_trained_model'],dense['PDCNet'], dense['optim_iter'], dense['local_optim_iter'],
            path_to_pre_trained_models="basicsr/models/densematching/pre_trained_models")
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            ##### META #####
            if epoch > 0:
            # inf_data*255.permute(d)
            # breakpoint()
                inf_data = train_data[0].permute(0,3,1,2).float() / 255.0
                model.feed_data(data={'lq': inf_data})
                out_data = model.test()
                train_data[0] = out_data.permute(0, 2, 3, 1).numpy() * 255
                train_data[0] = train_data[0].astype(np.uint8)
            ...
            train_data = DenseMatching(opt['densematching'], opt['datasets']['train'], network, estimate_uncertainty, train_data)
            # breakpoint()
            ...
            # validation
            if opt.get('val') is not None and (current_iter % opt['val']['val_freq'] == 0 or current_iter == 500):
